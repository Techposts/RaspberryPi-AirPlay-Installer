...
# ==============================================================================
# WordPress + Cloudflare Tunnel - Easy Interactive Installer
# ==============================================================================
# Uses cloudflared's built-in login for easy setup!
# ==============================================================================
...
echo -e "${C_CYAN}   WordPress + Cloudflare Tunnel - Easy Installer${C_RESET}"
echo -e "${C_GREEN}════════════════════════════════════════════════════════════${C_RESET}"
echo
...
apt-get update -qq || print_warning "Some updates failed"

print_info "Upgrading packages..."
DEBIAN_FRONTEND=noninteractive apt-get upgrade -y -qq || print_warning "Some upgrades failed"
...
a2enmod rewrite ssl headers expires || print_warning "Some modules already enabled"
...
mysql -u root -p"$MYSQL_ROOT_PASSWORD" <<'EOSQL' || print_warning "Some security steps skipped"
...
rm -f /tmp/latest.tar.gz /tmp/wordpress -rf
...
echo -e "${C_YELLOW}  IMPORTANT: Cloudflare Login Required${C_RESET}"
echo -e "${C_YELLOW}════════════════════════════════════════════════════════════${C_RESET}"
echo
print_info "A browser window will open for Cloudflare authentication"
print_info "If running on a headless server, you'll get a URL to visit"
echo
read -p "Press ENTER when ready to authenticate..." dummy
...
if [[ ! -f ~/.cloudflared/cert.pem ]]; then
    error_exit "Cloudflare login failed. Please try again."
fi
...
echo
print_info "Tunnel logs: sudo journalctl -u cloudflared -f"
echo
